package mk.ukim.finki.company_stock_microservice.repository;
...
import mk.ukim.finki.company_stock_microservice.repository.util.CSVReadingUtil;
...
public class StockRepository {
    ...
    public List<Observation> getRecordsFromTo(String company, LocalDate from, LocalDate to) {
        return companies.get(company)
                .stream()
                .filter(obs -> obs.getDate().isBefore(to.plusDays(1)) && obs.getDate().isAfter(from.minusDays(1)))
                .sorted()
                .toList();
    }
        ...
        try {
            ...
            for (int i = 0; i < valueNames.length; i++) {
                if (!ls.get(i + 1).trim().isBlank()) {
                    values.put(valueNames[i], FormatConvenienceUtil.numberFormat.parse(ls.get(i + 1)).doubleValue());
                }
            }
